...
const Context = ({ children }: AppContextProps) => {
  ...
  const newGame = () => {
    const random = getNewRandomList({ list: allBreeds, number: NUMBER_TO_GUESS })
  ...
  useEffect(() => {
    if (firstRender.current) {
      ...
      const fetchAllBreeds = async () => {
        ...
        if (status !== 'error') {
          const allBreedsFetched = Object.keys(message)
          setAllBreeds(allBreedsFetched)
        }
      }
      fetchAllBreeds()
    }
  ...
  useEffect(() => {
    if (currentBreed) {
      const fetchBreedImage = async () => {
        const { status, message } = await getDogImage(currentBreed)
        if (status !== 'error') {
          setImageUrl(message)
        }
      }
